...
llmDescription: "Introduction to Argyros, a swap aggregator on Fogo (Solana coming soon). Covers what Argyros is, its value props (best-price multi-hop routing, fast execution, simple 3-endpoint API), the endpoint table (GET /api/v1/quote, POST /api/v1/swap, POST /api/v1/instructions, GET /health), and quick cURL + TypeScript + Python examples for getting a SOL→USDC quote on Fogo. Also available as a self-hosted community binary (see /get-started/self-hosted) for operators who want full control with their own FluxRPC key and community license."
---
