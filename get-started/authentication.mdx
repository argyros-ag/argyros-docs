...
llmDescription: "How to authenticate with the Argyros API. Every request to https://api.argyros.xyz is authenticated with an API key (format argy_...) sent in the Authorization: Bearer header. Covers getting a key (free during beta via Discord), authenticating with cURL/TypeScript/Python, passing the key in the SDK (apiKey) and widget (api-key attribute), the WebSocket query-param form (/api/v1/stream?key=argy_...), 401/403 errors for missing or invalid keys, key security best practices, and a link to rate limits. Fogo; Solana coming soon."
icon: "key"
---
