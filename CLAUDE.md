...
Mintlify documentation site for **Argyros** — a swap aggregator / routing engine on **Fogo Chain** (Solana support is documented as "coming soon"). This is the public developer docs surface; it documents three sibling projects that live elsewhere in the `mono_argyros` umbrella:
...
- **sdk** (`@argyros/sdk`) — TypeScript client wrapping the API.
- **widget** (`@argyros/widget`) — the `argyros-swap` Web Component.
...
- Frontmatter fields in use: `title`, `sidebarTitle` (optional), `description`, and a custom **`llmDescription`** — a long, keyword-rich summary written specifically for LLM/AI consumption. When adding a page, follow the existing pattern and write an `llmDescription` that fully summarizes the page's content and surfaces (endpoints, code examples, product caveats like "Fogo live; Solana coming soon").
...
- `skill.md` (root) is a standalone **agent-skill definition** (`name: argyros-swap`) that teaches an AI agent to call the Argyros API. It duplicates the API surface in condensed form — when endpoints, params, or error behavior change, update it alongside the page content and `openapi.json`. `Assistant.md` is currently empty.
...
- Product framing is consistent across the docs: **Fogo is live; Solana is "coming soon."** Multi-hop routing spans Vortex, Fluxbeam, Fogo.fun, and Moonit. Keep new copy consistent with this.
